...
function Dashboard({ uid, demo, onSignOut }) {
  ...
  const archiveTrip = async () => {
    ...
    const ok = window.confirm(
      `Archive “${group.name}”?\n\nIt disappears from the app and stops being refreshed, but all its flights and load history stay stored. It can be restored later.`
    );
    if (!ok) return;
    try {
      await archiveGroupNow(group.id);          // hides it everywhere instantly
      enqueueArchiveGroup(group.id, group.name); // syncs the work PC when online
      setQueuedMsg(`Archived “${group.name}”.`);
    } catch (e) {
      setErr(e.message || 'Could not archive the trip.');
    }
  };
          ...
          {(() => {
            ...
            const inb = (
              <Section title={`Inbound${inbound[0] ? ' · ' + inbound[0].origin + ' → ' + inbound[0].destination : ''}`}
                flights={inbound} cols={cols} isPhone={isPhone} inline={inline} app={shared} myCode={myCode} myDoj={myDoj} />
            );
            return inline
              ? <div className="flex flex-wrap justify-center items-start gap-x-10">{out}{inb}</div>
              : <>{out}{inb}</>;
          })()}
...
function CommandStrip({ commands, onRerun }) {
  ...
  const [openPref, setOpenPref] = useState(null); // null = auto (open only while active)
  useEffect(() => {
  ...
  const summary = [
    ...
    counts.stale ? `${counts.stale} stuck` : null,
    counts.pending ? `${counts.pending} queued` : null,
    counts.error ? `${counts.error} failed` : null,
  ].filter(Boolean).join(' · ') || `${counts.done} done`;

  return (
    <div className="bg-white rounded-2xl border border-stone-200 mb-4">
      <button onClick={() => setOpenPref(!open)}
        className="w-full flex items-center gap-2 px-3 py-2.5 text-left">
        <span className="text-[10px] font-semibold text-stone-500 uppercase tracking-wide">Activity</span>
        {(counts.running > 0 || counts.stale > 0) && (
          <span className={'inline-block w-2 h-2 rounded-full ' + (counts.stale ? 'bg-amber-500' : 'bg-blue-500 animate-pulse')} />
        )}
        <span className="text-[11px] text-stone-400 truncate">{summary}</span>
      ...
      {open && (
        <div className="px-3 pb-3">
          ...
          <p className="mt-2 text-[10px] text-stone-400">Last {commands.length} commands. Stuck commands also resume automatically when your service restarts.</p>
        </div>
      )}
    </div>
  );
